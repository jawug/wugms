...
if (!empty($_POST)) {
    ...
    if ($iserror === false) {
        if (empty($_POST['delete_siteid'])) {
            ...
            if (!isset($_SESSION["id"])) {
                ...
                $snquery        = "SELECT 1 FROM tbl_base_sites WHERE siteID = :siteID and idSite_Owner = :user_ID ;";
                $snquery_params = array(
                    ':siteID' => $_POST['delete_siteid'],
                    ':user_ID' => $_SESSION["id"]
                );
                try {
    ...
    if ($iserror === false) {
        ...
        $query = "DELETE FROM tbl_base_sites WHERE siteID = :siteID AND idSite_Owner = :user_ID; ;";
        ...
        try {
            ...
            wugmsaudit("user", "delete", "sites", "Deleted site - " . $_POST['delete_site_name'] . " UID" . $_SESSION["id"]);
