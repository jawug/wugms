...
if (!empty($_POST)) {
    ...
    if (empty($_POST['site_name'])) {
        ...
        if (!preg_match("/^[A-Za-z0-9]{3,20}$/i", $_POST['site_name'])) {
            // Set the error flag as we do not want the process to run further
            $iserror = true;
            // Set the error code
    ...
    if ($iserror === false) {
        if (empty($_POST['height'])) {
            ...
            if (!($_POST['height'] >= 1) or ! ($_POST['height'] <= 20)) {
                // Set the error flag as we do not want the process to run further
                $iserror = true;
                // Set the error code
    ...
    if ($iserror === false) {
        ...
        $query = "insert into tbl_base_sites (
                    Name
                    ,longitude
                    ,latitude
                    ,height
        ...
        try {
            ...
            wugmsaudit("user", "add", "sites", "Created site - " . $_POST['site_name'] . " UID" . $_SESSION["id"]);
