...
if (isValueInRoleArray($_SESSION["roles"], "admin") && isset($_GET['sn'])) {
	...
	wugmsaudit("ADMIN", "rb_review", "review_data", $_SESSION["irc_nick"] . " (" . $_SESSION["id"] .  ") viewed tbl_base_rb_ip_route_config for " . $_GET['sn']);
		
    $wugms_table_user_rb_header_row = '';
    $outp                           = '';
    
    $wugms_table_user_rb_header_query = "
SELECT a.dst_addressStr,
       a.gateway,
       a.scope,
       a.distance,
       a.target_scope,
       ...
       a.comment
  FROM tbl_base_rb_ip_route_config a
 WHERE Serial_Number = :rb_id
ORDER BY a.dst_address;";
    ...
    if ($wugms_table_user_rb_header_row) {
        ...
        foreach ($wugms_table_user_rb_header_row as $x) {
            ...
            $outp .= '{"dst_addressStr":"' . $x["dst_addressStr"] . '",';
            $outp .= '"gateway":"' . $x["gateway"] . '",';
            $outp .= '"scope":"' . $x["scope"] . '",';
            $outp .= '"distance":"' . $x["distance"] . '",';
            $outp .= '"target_scope":"' . $x["target_scope"] . '",';
