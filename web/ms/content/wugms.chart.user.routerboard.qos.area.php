...
$wugms_chart_qos_query = "
SELECT (UNIX_TIMESTAMP(c.curr_fdate) + 7200) * 1000 'idate',
       c.curr_name,
       if(c.curr_bytes >= p.prev_bytes,
          c.curr_bytes - p.prev_bytes,
          c.curr_bytes)
          'bytes'
  FROM (SELECT DATE_FORMAT(l.RDate, '%Y-%m-%d %H:00') 'curr_fdate',
               l.Name 'curr_name',
               l.Bytes 'curr_bytes'
          FROM tbl_base_snmp_mikrotik_queuetree l
         WHERE     l.RDate > SUBdate(NOW(), INTERVAL 7 DAY)
               AND l.IPAddressStr = :user_ip
               AND l.Flow = :flow_type
        ORDER BY l.rdate, l.name) AS c,
       (SELECT DATE_FORMAT(l.RDate, '%Y-%m-%d %H:00') 'prev_fdate',
               DATE_FORMAT(adddate(l.RDate, INTERVAL 1 HOUR),
                           '%Y-%m-%d %H:00')
                  'mod_fdate',
               l.Name 'prev_name',
               l.Bytes 'prev_bytes'
         ...
         WHERE     l.RDate > SUBdate(NOW(), INTERVAL 7 DAY)
               AND l.IPAddressStr = :user_ip
               AND l.Flow = :flow_type
        ORDER BY l.rdate, l.name) AS p
 WHERE c.curr_name = p.prev_name AND c.curr_fdate = p.mod_fdate;
";
...
//				$series1['name'][]=array($x['curr_name'], array($x['idate'],$x['bytes']));
				...
				//$result            = array();
//                array_push($result, $series1);
			$res[$x['curr_name']][] =  array($x['idate'], $x['bytes']);
			}
			foreach ($res as $k=>$i) {
				$tmp['name']=$k;
				$tmp['data']=$i;
