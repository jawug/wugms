...
$wugms_chart_qos_query = "
SELECT q.flow, if(q.bytes > 0, q.bytes, 1) 'bytes'
  FROM (SELECT concat(x.current_flow, '__', x.current_host) 'flow',
               if(x.current_bytes >= y.previous_bytes,
                  x.current_bytes - y.previous_bytes,
                  x.current_bytes)
                  'bytes'
          FROM (SELECT a.Flow 'previous_flow', sum(a.Bytes) 'previous_bytes'
                  FROM tbl_base_snmp_mikrotik_queuetree a
                ...
                GROUP BY a.IPAddressStr, a.Flow) AS y,
               (SELECT a.IPAddressStr 'current_host',
                       a.Flow 'current_flow',
                 ...
                 WHERE     (a.RDate BETWEEN DATE_FORMAT(
                       ...
                       AND a.flow <> ''
                       AND a.IPAddressStr = :user_ip
                GROUP BY a.IPAddressStr, a.Flow) AS x
         WHERE x.current_flow = y.previous_flow) AS q;
";
