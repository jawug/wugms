...
$wifi_ssid_ap_58ghz_row = '';
	/* Check if a serial number was supplied */
        $wifi_ssid_ap_58ghz_query = "
SELECT count(*) 'counter'
 ...
 WHERE     x.Serial_Number = a.Serial_Number
       ...
       AND a.disabled = '0'
       AND wifi_band LIKE '5%';";
        
		/* Try and execute the SQL */
        $wifi_ssid_ap_58ghz_stmt = $db->prepare($wifi_ssid_ap_58ghz_query);
        if ($wifi_ssid_ap_58ghz_stmt->execute()) {
            $wifi_ssid_ap_58ghz_row = $wifi_ssid_ap_58ghz_stmt->fetchAll();
        }
		
        if ($wifi_ssid_ap_58ghz_row) {
			/* There was feedback from the sql query so pack up thos results */
            foreach ($wifi_ssid_ap_58ghz_row as $key => $value) {
                $data[] = array(
